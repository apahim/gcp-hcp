...
**Implementation**: [gecko PR #5](https://github.com/openshift-online/gecko/pull/5), [api-aggregation.md](https://github.com/openshift-online/gecko/blob/api-aggregation-proposal/api-aggregation.md)
...
* Internal traffic depends on GKE kube-apiserver availability — if the control plane is down, controllers cannot reach gecko (public API unaffected, requests persist in datastore, controllers catch up on recovery)
...
* **Resiliency**: GKE regional SLA is 99.95%. If the kube-apiserver is unavailable, internal traffic is delayed but not lost — requests persist in the datastore and controllers catch up on recovery. The public API path is completely independent.
...
* Dual-mode server supports `--aggregated` flag for production and standalone mode (no TLS, no auth) for local development.
